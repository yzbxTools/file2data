...
def copy_files(file_paths, src_root_dir, dst_root_dir, num_workers=4):
    ...
    """
    func = partial(
        copy_single_file, src_root_dir=src_root_dir, dst_root_dir=dst_root_dir
    )

    # 使用多线程复制图片
    results = parallelise(
        func, file_paths, num_workers=num_workers, task_type="io_bound"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_txt", type=str, required=True, help="txt file containing file paths"
    )
    parser.add_argument(
        "--src_root_dir", type=str, required=True, help="source root directory"
    )
    parser.add_argument(
        "--dst_root_dir", type=str, required=True, help="destination root directory"
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    parser.add_argument(
        "--output_txt", type=str, help="output txt file with new file paths (optional)"
    )
    ...
    # 复制图片
    print(
        f"Copying files from {args.src_root_dir} to {args.dst_root_dir} with {args.num_workers} workers..."
    )
    valid_paths, invalid_count = copy_files(
        file_paths, args.src_root_dir, args.dst_root_dir, args.num_workers
    )
